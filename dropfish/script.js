...
function addHistory(text,type='event',detail='',meta={}) {
  const defaultNumbered=['fish','bonus','epic','legendary','trash'].includes(type);
...
function fishTitleText(fish) {
  ...
  const hasPositiveBoost=Boolean(fish?.flipperImpact || fish?.maskImpact || fish?.scubaImpact || fish?.leviathanImpact || essenceApplied);
...
function renderHistory() {
  ...
  $('historyList').innerHTML=state.history.map((h)=>{
    ...
    return `<li data-history-id="${h.id}" class="history-item type-${h.type}${h.eaten?' is-eaten':''}${h.stolen?' is-stolen':''}${h.transmutation?' is-transmutation':''}${weatherClass}${h.id===lastAnimatedHistoryId?' is-new':''}"><strong>${mainLine}</strong>${h.detail?`<small class="history-detail">${h.detail}</small>`:''}${impactNote?`<small class="history-impact-note">${impactNote}</small>`:''}${essenceImpact}${leviathanImpact}${flipperImpact}${maskImpact}${scubaImpact}${gearBonusStatus}${scubaBonusStatus}${transmutation}${embeddedFish}${rowAmbient}</li>`;
  }).join('');
