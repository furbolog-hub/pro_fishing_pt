...
const BASE_WEIGHTS = { normal:58.8, heavy:8, giant:2, trash:18, bonus:8, epic:1.2, legendary:0.25 };
const COIN_CATCH_CHANCE = 0.025;
const COIN_TYPES = Object.freeze([
  {key:'copper', name:'Медная монета', add:10, luck:0.12, weight:60},
  {key:'silver', name:'Серебряная монета', add:20, luck:0.15, weight:30},
  {key:'gold', name:'Золотая монета', add:30, luck:0.20, weight:10}
]);
function coinIconMarkup(type, extraClass='') {
  const safeType=['copper','silver','gold'].includes(type)?type:'copper';
  return `<span class="coin-icon coin-${safeType}${extraClass?` ${extraClass}`:''}" aria-hidden="true"><span class="coin-shine"></span></span>`;
}
...
function initialState() {
  return {
    ...
    arcadeCaughtCount:0, arcadeLastSpawnCast:-1, pendingSeagulls:[], coins:[], pendingCoinBoosts:[], sessionDate:null, finalResult:null
  };
}
...
function addHistory(text,type='event',detail='',meta={}) {
  const defaultNumbered=['fish','bonus','epic','legendary','trash','coin'].includes(type);
...
function fishTitleText(fish) {
  ...
  const hasPositiveBoost=Boolean(fish?.flipperImpact || fish?.maskImpact || fish?.scubaImpact || fish?.leviathanImpact || fish?.goldenHourImpact || essenceApplied || hasCoinBoost);
  const limitedBase=Number.isFinite(fish.debuffBaseWeight) ? fish.debuffBaseWeight : fish.weight;
  const normalBase=Number.isFinite(fish.originalWeight) ? fish.originalWeight : fish.weight;
  const caughtWeight=fish.debuffLimited ? limitedBase : normalBase;
  const hasChangedWeight=!fish.debuffLimited && hasPositiveBoost && Math.abs(fish.weight-caughtWeight)>=0.05;
...
function renderGoldenHourImpact(fish) {
  if (!fish?.goldenHourImpact) return '';
  const impact=fish.goldenHourImpact;
  return `<div class="gear-impact golden-hour-impact"><span class="gear-icon">🌅</span><span><strong>Золотой час +${impact.plus} кг</strong> <span class="gear-arrow">→</span> <span class="gear-target">${capitalize(fish.name)}</span><small>${kg(impact.before)} + ${impact.plus} кг = <b class="gear-result">${kg(impact.after)}</b></small></span></div>`;
}

function renderLeviathanImpact(fish) {
  if (!fish?.leviathanImpact) return '';
  const impact=fish.leviathanImpact;
  return `<div class="gear-impact leviathan-impact"><span class="gear-icon">🐉</span><span><strong>Чешуя Левиафана +${impact.plus} кг</strong> <span class="gear-arrow">→</span> <span class="gear-target">${capitalize(fish.name)}</span><small>${kg(impact.before)} + ${impact.plus} кг = <b class="gear-result">${kg(impact.after)}</b></small></span></div>`;
}
...
function renderCoinAction(row) {
  if (row.type!=='coin' || !row.coinId) return '';
  const coin=state.coins.find(item=>item.id===row.coinId);
  if (!coin) return '';
  const disabled=state.finished || coin.used || coin.expired;
  const label=coin.used?(coin.success?'Использовано':'Не повезло'):(coin.expired?'Недоступно':'Бросить на удачу');
  return `<button type="button" class="coin-luck-btn${disabled?' is-disabled':''}" data-coin-id="${coin.id}" ${disabled?'disabled':''}>${label}</button>`;
}
...
function addFishHistory(fish, source) {
  const giant=fish.category==='giant';
  addHistory(`${giant?'🏆 ':''}${capitalize(fish.name)}${giant?' 💪🏼':''} — ${kg(fish.weight)}`,'fish',`${source}${fish.tags.length?` • ${fish.tags.join(' • ')}`:''}`,{fishId:fish.id,eaten:Boolean(fish.removed),arcade:Boolean(fish.arcadeCatch),numbered:source==='Заброс'});
}
function toast(text) {
...
function currentDebuffEventChance() {
  if (state.weather==='storm') return 0;
  if (state.weather==='golden') return 0.019;
  if (['sunny','rain','calm'].includes(state.weather)) return 0.065;
  return 0.0475;
}
function maybeTriggerDebuffEvent() {
  if (chance(currentDebuffEventChance())) processDebuff();
}
...
function processCoinCatch() {
  const type=chooseCoinType();
  const coin={id:uid(), type:type.key, name:type.name, add:type.add, luck:type.luck, used:false, success:false, applied:false, expired:false, historyRowId:null};
...
function useCoin(coinId) {
  ...
  if (!coin || coin.used || coin.expired || state.finished) return;
  coin.used=true;
...
function makeFish(category='normal', source='Заброс', direct=true, options={}) {
  ...
  if (!state.megalodon && (activeDebuff('Рак') || activeDebuff('Утка'))) {
    ...
    f.weight=rand1(.1,max); f.debuffBaseWeight=f.weight; f.debuffLimited=true; f.tags.push(`ограничение до ${kg(max)}`);
  }
  if (state.weather==='golden') { const plus=Math.floor(rand(1,5)); const before=f.weight; f.weight=round1(f.weight+plus); f.goldenHourImpact={before,after:f.weight,plus}; f.tags.push(`<span class="gold-add">+${plus} кг</span>`); }
...
function processEpic(name=pick(DATA.epics), fromAngus=false) {
  ...
  const epicRow = addHistory(name,'epic',fromAngus?'(Дар старины Ангуса)':'',{numbered:!fromAngus});
...
function processLegendary(name=pick(DATA.legendary), fromAngus=false) {
  ...
  if (fromAngus) state.angusGift=true;
  addHistory(name,'legendary',fromAngus?'(Дар старины Ангуса)':'',{numbered:!fromAngus});
...
function renderHistory() {
  const icons={fish:'🐟',bonus:'✅',debuff:'🛑',epic:'💜',legendary:'🧡',trash:'🔘',coin:'🪙',weather:'⚠️',angus:'🧔'};
  ...
  $('historyList').innerHTML=state.history.map((h)=>{
    ...
    const icon=h.type==='fish'&&h.eaten?'💀':h.type==='fish'&&h.stolen?'❌':h.type==='fish'&&h.arcade?'🐠':h.type==='coin'?coinIconMarkup(coinForRow?.type||'copper'):(entityBasedIcon||icons[h.type]||'');
    ...
    const mainLine=h.transmutation?`${numberPrefix}🧡 Трансмутация хлама`:`${numberPrefix}${icon} ${visibleText}`;
    const goldenHourImpact=fish?renderGoldenHourImpact(fish):'';
    ...
    return `<li data-history-id="${h.id}" class="history-item type-${h.type}${h.eaten?' is-eaten':''}${h.stolen?' is-stolen':''}${h.transmutation?' is-transmutation':''}${weatherClass}${h.id===lastAnimatedHistoryId?' is-new':''}"><strong>${mainLine}</strong>${h.detail?`<small class="history-detail">${h.detail}</small>`:''}${impactNote?`<small class="history-impact-note">${impactNote}</small>`:''}${essenceImpact}${goldenHourImpact}${leviathanImpact}${coinImpacts}${flipperImpact}${maskImpact}${scubaImpact}${gearBonusStatus}${scubaBonusStatus}${coinAction}${transmutation}${embeddedFish}${rowAmbient}</li>`;
  }).join('');
...
$('historyList').addEventListener('click',event=>{
  const button=event.target.closest('.coin-luck-btn[data-coin-id]');
  if (button) useCoin(button.dataset.coinId);
});
