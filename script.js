...
function maybeScheduleArcadeAfterCast() {
  ...
  if ((state.castClicks - (state.arcadeLastSpawnCast ?? -1)) < 2) return;
  if (!chance(ARCADE_TRIGGER_CHANCE)) return;
...
function render() {
  ...
  state.bonuses.forEach(b=>effects.push({label:`🎁 ${b.name}${state.disabledBonusIds.has(b.id)?' (отключён)':''}`,kind:'bonus'}));
  state.artifacts.forEach(a=>effects.push({label:`${a.tier==='legendary'?'🟠':'🟣'} ${a.name}`,kind:a.tier}));
  state.debuffs.forEach(d=>effects.push({label:`🔴 ${d.name}${d.active?'':' (нейтрализован)'}`,kind:'debuff'}));
...
function renderHistory() {
  ...
  $('historyList').innerHTML=state.history.map((h,i)=>{
    const icon=h.type==='fish'&&h.eaten?'💀':h.type==='fish'&&h.stolen?'❌':h.type==='fish'&&h.arcade?'🐠':(icons[h.type]||'');
    ...
    const mainLine=h.transmutation?`${i+1}. 🧡 Трансмутация хлама`:`${i+1}. ${icon} ${h.text}`;
    return `<li data-history-id="${h.id}" class="history-item type-${h.type}${h.eaten?' is-eaten':''}${h.stolen?' is-stolen':''}${h.transmutation?' is-transmutation':''}${weatherClass}${h.id===lastAnimatedHistoryId?' is-new':''}"><strong>${mainLine}</strong>${h.detail?`<small class="history-detail">${h.detail}</small>`:''}${impactNote?`<small class="history-impact-note">${impactNote}</small>`:''}${transmutation}${embeddedFish}${rowAmbient}</li>`;
  }).join('');
...
function openGuide(tab='Погода') {
  ...
  const renderItems=(query='')=>{
    ...
    const cards=items.map(([title,text,extraClass])=>`<article class="${extraClass||''}"><h3 class="${colorClass}">${title}</h3><p>${text}</p></article>`).join('');
