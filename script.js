...
function renderHistory() {
  ...
  $('historyList').innerHTML=state.history.map((h,i)=>{
    ...
    return `<li data-history-id="${h.id}" class="history-item type-${h.type}${h.eaten?' is-eaten':''}${h.stolen?' is-stolen':''}${weatherClass}${h.id===lastAnimatedHistoryId?' is-new':''}"><strong>${i+1}. ${icon} ${h.text}</strong>${h.detail?`<small class="history-detail">${h.detail}</small>`:''}${embeddedFish}${fishSilhouette}</li>`;
  }).join('');
