...
function addHistory(text,type='event',detail='') {
  const row={id:uid(),text,type,detail}; state.history.push(row); lastAnimatedHistoryId=row.id; renderHistory();
}
function toast(text) {
...
function makeFish(category='normal', source='Заброс', direct=true) {
  ...
  if (activeDebuff('Касатка') && f.weight>=5.5 && !state.megalodon) {
    f.removed=true; state.eaten.push(f); state.fishLostToDebuffs=true; addHistory(`🐋 Касатка съела: ${f.name} (${kg(f.weight)})`,'debuff'); return f;
  }
  state.fish.push(f);
  if (direct && category==='heavy') state.directHeavy=true;
  if (direct && category==='giant') state.directGiant=true;
  addHistory(`${giant?'🏆 ':''}${capitalize(f.name)}${giant?' 💪🏼':''} — ${kg(f.weight)}`,'fish',`${source}${f.tags.length?` • ${f.tags.join(' • ')}`:''}`);
...
function processTrash() {
  ...
  if (hasBonus('Счастливый поплавок')) {
    const restore=state.nautilus?2:1; state.castsLeft+=restore; state.luckyFloatSaves++; addHistory(`Счастливый поплавок вернул ${restore} заброс${restore===1?'':'а'}`,'bonus');
  }
}
...
function processDebuff(forcedName=null) {
  const name=forcedName||chooseDebuff(); state.receivedDebuffCount++; if(!state.receivedDebuffNames.includes(name))state.receivedDebuffNames.push(name); state.sessionCategories.debuff=true;
  if (state.megalodon) { addHistory(`${name} нейтрализован Плавником мегалодона`,'debuff'); return; }
  if (diverBlocks(name)) { state.blockedDebuffCount++; addHistory(`${name} заблокирован Снаряжением дайвера`,'bonus'); return; }
  playSound('debuff');
  showDebuffEffect(name);
  ...
  if (name==='Чайка') {
    ...
    addHistory(`Чайка украла: ${victim.name} (${kg(victim.weight)})`,'debuff');
  }
  if (name==='Осьминог') {
  ...
  if (name==='Касатка') {
    state.fish.filter(f=>!f.removed&&f.weight>=5.5).forEach(f=>{f.removed=true;state.eaten.push(f);state.fishLostToDebuffs=true;});
...
function processLegendary(name=pick(DATA.legendary), fromAngus=false) {
  ...
  if (name==='Игральная кость') showChoice('Игральная кость','Выберите один эффект:', ['+5 забросов','×5 финальный вес'], choice=>{ if(choice.startsWith('+')){state.castsLeft+=5;state.diceExtraCasts=true;}else{state.diceFinalMultiplier*=5;state.diceWeightMultiplier=true;} addHistory(`Игральная кость: ${choice}`,'legendary');render(); });
...
function encounterAngus(fromCompass=false) {
  ...
  if (chance(.05)) {
    ...
    state.angusGiantGift=true; addHistory('У Ангуса нет артефакта, но он добавляет гиганта','event'); makeFish('giant','Старина Ангус',false);
  }
}
...
function renderHistory() {
  ...
  $('historyList').innerHTML=state.history.map((h,i)=>`<li data-history-id="${h.id}" class="history-item type-${h.type}${h.id===lastAnimatedHistoryId?' is-new':''}"><strong>${i+1}. ${icons[h.type] || ''} ${h.text}</strong>${h.detail?`<small>${h.detail}</small>`:''}</li>`).join('');
